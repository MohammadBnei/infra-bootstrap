...
| Secrets | Infisical (project `infra-bootstrap-1-ge1`, id `8a3fa54f-be22-488a-bf51-55158f65c0f2`, domain `https://infisical.bnei.dev`, env `dev` — see `docs/secrets.md`). Values shared by more than one app come from the separate **`platform-commons`** project via `common-app-chart`'s `sharedSecrets.keys`, never by copying into a second per-app project — ADR-0049. The source project is a literal in the template, not a value: app `values.yaml` lives in repos this one doesn't review, and `template.data` filters the response without narrowing the identity's grant. Not inherited by `hooks:`/`oneOffJobs:` (PreSync runs before Sync). |
