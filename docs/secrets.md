...
| actions-runner | `actions-runner-x-qbo` / `b9043dd5-2956-43e4-9f1b-e9bf7a8e1edc` | `ACCESS_TOKEN` (GitHub fine-grained PAT, `Administration: Read and write`, **the repo owner's** — registers/deregisters self-hosted runners via the Actions runners API). Scoped to `vos-monolith` **+ `editable-blog`** — shared by *both* runner pods, so revoking it stops `oneOffJobs` as well as image builds. It cannot be a bot's: runner registration needs the repo `admin` role, and personal-account repos have only owner + write-collaborator, with granular roles being an organization feature (ADR-0034) | `gitops/platform/actions-runner/infisicalsecret.yaml` → `deployment.yaml`'s `envFrom`; `gitops/platform/build-runner/infisicalsecret.yaml` → an explicit `secretKeyRef` (**not** `envFrom` — that pod executes app-repo `Dockerfile`s, so it gets one key, not whatever else lands in the project) |
