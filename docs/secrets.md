...
| `DBUSER_WIRD_PASSWORD` | `dbuser_wird` — Wird's database user, owner of `wirddb` and of the `jidhr` schema in it (ADR-0050). **The PLAINTEXT**, same arrangement as `DBUSER_AUTHENTIK_PASSWORD` above: `pigsty/pigsty.yml` commits only the SCRAM-SHA-256 *verifier*, so rotation must regenerate both halves together. There is deliberately **no `WIRD_OIDC_CLIENT_ID` row** — Wird is a public OIDC client, and a public client's id ships inside the APK and the JS bundle, so it is committed in `gitops/bootstrap/authentik-blueprint-wird.yaml` rather than stored here | agent (random 32B urlsafe) | `pigsty/pigsty.yml` `pg_users`; `gitops/bootstrap/wird-secret.yaml`. The role and `wirddb` **exist** — created 2026-09-22, see `docs/bootstrap-test-notes.md`. The pod's route to them is `gitops/bootstrap/wird-secret.yaml`, which **assembles** `DATABASE_URL=postgres://dbuser_wird:<this plaintext>@postgres.bnei.lan:5432/wirddb` from this one row into the `wird-config` Secret that Wird's `helm/values.yaml` mounts by `envFrom` (the HA VIP by name, as ADR-0039 uses it, not a node IP; no `sslmode` — pgx v5 defaults to `prefer` and nothing here has measured whether this cluster's self-signed CA satisfies `verify-full`, so do not write one in without testing it). There is deliberately **no** second, per-app Infisical project holding a composed DSN: that would put this password in two entries with a hand-copy between them, the `WEDDING_WALL_S3_*` trap above. `common-app-chart`'s `sharedSecrets` genuinely cannot carry it (hardcoded to `platform-commons-76pb`), which is why the assembly lives in this repo rather than in Wird's values.yaml. Because it lands in a URL's userinfo unescaped, a rotation to a value containing `@`, `:` or `/` silently yields a different, wrong DSN — keep it urlsafe or percent-encode it there. Deployed and serving 2026-09-24 |
| `SMTP_HOST` / `SMTP_PORT` | Outbound mail relay for authentik (ADR-0050) — SMTP2GO, `mail-eu.smtp2go.com:587`. Not secret in themselves, but they live here because `authentik.existingSecret` is all-or-nothing, the same reason the Postgres host and port do. `ente-uovi` has five keys spelled identically (and the local `register-repos.env` three of them), but they are a **different SMTP2GO account** — same relay host, separate credentials. They rotate independently; the shared spelling is a trap, not a mirror | operator (from the mail provider) | `gitops/bootstrap/authentik-secret.yaml` → `AUTHENTIK_EMAIL__HOST` / `__PORT` |
| `SMTP_USERNAME` / `SMTP_PASSWORD` | The relay credential. **Load-bearing beyond Wird**: with these unset authentik falls back to `localhost:25` and silently drops every message — enrollment verification, password recovery and email MFA alike, with no UI error | operator (from the mail provider) | `gitops/bootstrap/authentik-secret.yaml` → `AUTHENTIK_EMAIL__USERNAME` / `__PASSWORD` |
| `SMTP_EMAIL` | From address for authentik's mail. Spelled `SMTP_EMAIL` rather than `SMTP_FROM` only because `ente-uovi` spells it that way — the value is authentik's own, not ente's | operator | `gitops/bootstrap/authentik-secret.yaml` → `AUTHENTIK_EMAIL__FROM` |
