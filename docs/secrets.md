...
| `SMTP_HOST` / `SMTP_PORT` | Outbound mail relay for authentik (ADR-0050) — SMTP2GO, `mail-eu.smtp2go.com:587`. Not secret in themselves, but they live here because `authentik.existingSecret` is all-or-nothing, the same reason the Postgres host and port do. **A second copy of these five lives in `ente-uovi`** (and three of them in the local `register-repos.env`); the names are identical across projects so a rotation is a find-and-replace, but it must touch every copy | operator (from the mail provider) | `gitops/bootstrap/authentik-secret.yaml` → `AUTHENTIK_EMAIL__HOST` / `__PORT` |
...
| `SMTP_EMAIL` | From address — `server@bnei.dev`. Named `SMTP_EMAIL` rather than `SMTP_FROM` to match the `ente-uovi` copy exactly | operator | `gitops/bootstrap/authentik-secret.yaml` → `AUTHENTIK_EMAIL__FROM` |
