...
| `UKUBI_BOT_GH_PAT` | Wide GitHub PAT for the dedicated `argocd-ukubi-bot` account, which is added as a **collaborator** on repos it needs to act on. Distinct from `GITHUB_APPS_PAT` above, which is read-only and only lets ArgoCD *clone* — this one can act. Note the ceiling: registering a self-hosted runner needs the bot to hold the repo **Admin** role; Write and even Maintain get a 403 from `actions/runners/registration-token` | user (github.com, `argocd-ukubi-bot`) | `gitops/platform/build-runner/` (duplicated into the `actions-runner` project — see the per-app table below, and **rotate both copies together**) |
...
| actions-runner | `actions-runner-x-qbo` / `b9043dd5-2956-43e4-9f1b-e9bf7a8e1edc` | `ACCESS_TOKEN` (GitHub fine-grained PAT, `Administration: Read and write`, scoped to `vos-monolith` — registers/deregisters the self-hosted runner via the Actions runners API); `UKUBI_BOT_GH_PAT` (wide PAT for the `argocd-ukubi-bot` account — **a duplicate** of the same key in `infra-bootstrap-1-ge1`, copied here so the build runner's namespace Secret never contains `PVE_API_TOKEN`/`K8S_BREAK_GLASS_TOKEN`/`GARAGE_ROOT_TOKEN`. **Rotate both copies together.** Requires the bot to hold the repo **Admin** role, not Write — GitHub gates runner registration on `admin`, `maintain` is refused; ADR-0034 records the accepted trade-off) | `gitops/platform/actions-runner/infisicalsecret.yaml` → `deployment.yaml`'s `envFrom`; `gitops/platform/build-runner/infisicalsecret.yaml` → an explicit `secretKeyRef` mapping `UKUBI_BOT_GH_PAT` onto `ACCESS_TOKEN` (**not** `envFrom` — that pod executes app-repo `Dockerfile`s, so it gets one key, not the project) |
