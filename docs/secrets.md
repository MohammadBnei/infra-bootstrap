...
| ukubi-stt | `ukubi-stt-bhr-m` / `639e5437-3257-4694-8d73-dae647717b18` | `STT_AUTH_TOKEN` (shared bearer token for the gRPC service, ADR-0044 Decision 5 — 384 bits from `openssl rand -base64 48`, URL-safe alphabet, **generated not chosen**: rate limiting is per-IP with no failed-auth lockout, so a short token is brute-forceable from many source IPs, and `stt.bnei.dev` is in CT logs minutes after issuance). **Rotating it breaks browsers silently** — `autoReload: true` restarts the pod and every browser client then 401s with no re-prompt, because the token sits in `localStorage`; the browser client must ship a 401 handler that clears storage and re-prompts, or rotation is an outage nobody diagnoses. Note the registry credential for this repo is **not** here: like every build repo it is a native GitHub Actions secret (`REGISTRY_PASSWORD`), for the reason recorded against `ZOT_HTPASSWD` above | `ukubi-stt`'s `helm/values.yaml` (`infisical.enabled`, `projectSlug: ukubi-stt-bhr-m`) — not yet authored, Gate 0 is unresolved |
