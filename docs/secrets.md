...
| `DBUSER_WIRD_PASSWORD` | `dbuser_wird` — Wird's database user, owner of `wirddb` and of the `jidhr` schema in it (ADR-0050). **The PLAINTEXT**, same arrangement as `DBUSER_AUTHENTIK_PASSWORD` above: `pigsty/pigsty.yml` commits only the SCRAM-SHA-256 *verifier*, so rotation must regenerate both halves together. There is deliberately **no `WIRD_OIDC_CLIENT_ID` row** — Wird is a public OIDC client, and a public client's id ships inside the APK and the JS bundle, so it is committed in `gitops/bootstrap/authentik-blueprint-wird.yaml` rather than stored here | agent (random 32B urlsafe) | `pigsty/pigsty.yml` `pg_users`; Wird's own backend config. The role and `wirddb` **exist** — created 2026-09-22, see `docs/bootstrap-test-notes.md`. What does not exist yet is the pod's route to them: the API reads a single `DATABASE_URL`, and `common-app-chart`'s `sharedSecrets` cannot carry it (that block is hardcoded to `platform-commons-76pb`, and this project is deliberately unreadable from app namespaces). So wird needs its **own** Infisical project holding a composed `DATABASE_URL=postgres://dbuser_wird:<this plaintext>@postgres.bnei.lan:5432/wirddb` (the HA VIP by name, as ADR-0039 uses it, not a node IP; no `sslmode` — pgx v5 defaults to `prefer` and nothing here has measured whether this cluster's self-signed CA satisfies `verify-full`, so do not write one in without testing it), copied by hand out of here — the same manual split, and the same silent-failure trap, as `WEDDING_WALL_S3_*` above |
