...
| `SMTP_HOST` / `SMTP_PORT` | Outbound mail relay for authentik (ADR-0050). Not secret in themselves, but they live here because `authentik.existingSecret` is all-or-nothing — the same reason the Postgres host and port do | operator (from the mail provider) | `gitops/bootstrap/authentik-secret.yaml` → `AUTHENTIK_EMAIL__HOST` / `__PORT` |
...
| `SMTP_FROM` | Envelope/From address, e.g. `no-reply@bnei.dev`. Must be a sender the relay is authorised for, or mail is accepted and then dropped downstream | operator | `gitops/bootstrap/authentik-secret.yaml` → `AUTHENTIK_EMAIL__FROM` |
