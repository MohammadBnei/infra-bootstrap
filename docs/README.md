...
- [x] `runbook-authentik-identity.md` — operating the authentik identity layer (ADR-0039): blueprints, the `platform-admins` group both ArgoCD and Grafana read, the four-step propagation chain that nothing hot-reloads, and the field shapes verified against the running 2026.8.0. **Native OIDC tier live 2026-08-19** (ArgoCD, Grafana, and `fleet` natively inside `core` per [ADR-0041](adr/0041-fleet-native-oidc-not-forwardauth.md)); **forwardAuth tier live 2026-08-21**, gating the e2e preview hosts and `wedding.bnei.dev/admin`. Alertmanager, pgweb and Proxmox still to move, and `basic-admin-auth` still to retire ([#183](https://github.com/MohammadBnei/infra-bootstrap/issues/183)). **Public-client tier added 2026-09-22** ([ADR-0050](adr/0050-public-oidc-client-for-native-apps.md)): Wird is the first `client_type: public` client — PKCE enforced by an expression policy rather than a provider field, self-service enrollment into `wird-users`, and `core_default_app_access` flipped to `false` so an application with no binding now denies instead of admitting. That flag is the one piece of authentik state that is **not** a blueprint (`Tenant` is internally managed) and a Postgres restore silently reverts it — the runbook's last section is the check
- [x] `runbook-k9s-ops-hub.md` — driving ansible/pigsty/kubectl from the `k9s-dashboard` LXC instead of the workstation, for when the operator is off the LAN. **Built 2026-09-22.** Opt-in tooling in `k9s-dashboard-configure.yml` (`-e k9s_hub=true`), a project-scoped **read-only** Infisical service token (30-day expiry, nothing warns you), and no target SSH keys at rest — they are fetched per run and deleted. Note this box already holds a cluster-admin token and has root SSH exposed to the internet, so it raises what a compromise costs
- [x] `runbook-pgbackrest-s3-migration.md` — move pgBackRest off node-local repos onto the shared Garage `pg-backup` bucket, plus `restore_command`, per [ADR-0042](adr/0042-pgbackrest-shared-repo-and-restore-command.md). **Run 2026-08-26.** Repo is shared and verified; the full starve test is still outstanding — see the ADR's rollout note
