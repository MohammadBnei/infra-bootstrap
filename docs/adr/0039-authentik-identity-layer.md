...
| 3 — four access tiers | **Native OIDC and forwardAuth built; critical tier not.** Grafana and ArgoCD are federated, and so is `fleet` — but natively, inside `core`, per [ADR-0041](0041-fleet-native-oidc-not-forwardauth.md) rather than through a middleware. The forwardAuth tier landed 2026-08-21 (PRs #190/#194/#195): one domain-level proxy provider, `Middleware/authentik-forwardauth` in `default`, currently gating the e2e preview hosts, `wedding.bnei.dev/admin` and `blog.bnei.dev/linkedin`. One `forward_domain` provider covers the whole zone, so a new host needs no blueprint entry — only the `middlewares:` ref, and no `headers` middleware ahead of it that rewrites `X-Forwarded-Host` (infra-bootstrap #240). Alertmanager, pgweb and Proxmox are still on the old auth — each is one `middlewares:` entry away (infra-bootstrap #183, agent-fleet #209) |
