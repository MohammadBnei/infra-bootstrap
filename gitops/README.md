...
- **No cert-manager** — TLS via Traefik ACME (**TLS-ALPN-01**, resolver `le`), `acme.json` on a PVC — [ADR-0001](../docs/adr/0001-ingress-traefik-ingressroute-over-gateway-api.md). A second resolver `le-dns` (DNS-01 via Cloudflare) exists **only** to issue the `*.e2e.bnei.dev` wildcard — [ADR-0032](../docs/adr/0032-dns-to-cloudflare-and-dns01-wildcard.md)
